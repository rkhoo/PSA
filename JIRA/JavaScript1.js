...
upstream.custom.TaskScript = (function ($) {
    ...
    return {
        ...
        ProcessTaskState: function (task, user, state, callbacks) {
            _logger.debug("TaskScript", "Entering ProcessTaskState with state: {0}", state);

            var phoneNumber;

            // This is an example of how to trigger an outbound dial. Note that outbound phone calls can only be made if agent is in a NOT READY state.
            // Also note that this phone number will be passed into the dial rules script
            //if (state === 'Accept' && task.getChannelType() === upstream.taskservices.ChannelType.Email) {
            //    callbacks.Dial.call(this, phoneNumber, _onDialSuccess, _onDialError);
            //}

            // This is an example of how to trigger a "done" of a task
            // NOTE: should only be called when we know a task is active (i.e. completing a task while it is presenting is not a valid scenario)
            //if (state === 'PhoneDisconnect' && (task.getChannelType() === upstream.taskservices.ChannelType.Inbound || task.getChannelType() === upstream.taskservices.ChannelType.Outbound)) {
